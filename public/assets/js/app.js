...
$.ajax({
  ...
  if (response.ok) {
    var result = [];
    Object.entries(response.messages).forEach(function (item) {
      var unixtime = item[1].ts;
      var datetime = new Date(unixtime * 1000);
      var year = datetime.getFullYear();
      var month = datetime.getMonth() + 1;
      var date = datetime.getDate();
      var postdate = "".concat(year, "/").concat(month, "/").concat(date);

      if (!result[postdate]) {
        result[postdate] = item;
      } else {
        var indent = result[postdate].length;
